...
CONTAINER="phishaware"
IMAGE="saiyan/phishaware:latest"
...
if [[ ! $check_container == $CONTAINER ]]; then
	...
	docker create \
		--interactive --tty \
		--volume ${MOUNT_LOCATION}:/phishaware/auth/ \
		--network host \
		--name "${CONTAINER}" \
		"${IMAGE}"
fi
...
# docker run --rm -ti --network="host" -v ${MOUNT_LOCATION}:/phishaware/auth/ --name "$CONTAINER" "$IMAGE"
